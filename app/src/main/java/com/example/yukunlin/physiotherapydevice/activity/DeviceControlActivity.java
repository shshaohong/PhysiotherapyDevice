...
public class DeviceControlActivity extends BaseActivity
    ...
    private void write(final byte[] value) {
        ...
        bluetoothClient.write(mac, UUID.fromString(serUuid), UUID.fromString(charUuid), value,
                new BleWriteResponse() {
                    @Override
                    public void onResponse(int code) {
                        Log.e(TAG, "onResponse: " + "数据写入成功");
                    }
                });
    }
    ...
    private void connectStatus() {
        bluetoothClient.registerConnectStatusListener(mac, bleConnectStatusListener);
    }
    ...
    private BleConnectStatusListener bleConnectStatusListener = new BleConnectStatusListener() {
        ...
        public void onConnectStatusChanged(String mac, int status) {
            if (status == STATUS_CONNECTED) {
                ...
                bluetoothClient.connect(mac, new BleConnectResponse() {
                    @Override
                    public void onResponse(int code, BleGattProfile data) {
                        if (code == REQUEST_SUCCESS) {
//                            deviceOrder.setLength(0);
    ...
    private void receiveDataFromDevice() {
        bluetoothClient.notify(mac, UUID.fromString(serUuid), UUID.fromString(charUuid),
